    ...
    public int process(List<Update> updates) {
        updates.forEach(update -> {
            ...
            if (update.message().text().equals("/start")) {
                SendMessage message = new SendMessage(chatId, String.format("Привет, %s! Введите задачу в формате 01.01.2022 20:00 Сделать домашнюю работу", update.message().from().firstName()));
